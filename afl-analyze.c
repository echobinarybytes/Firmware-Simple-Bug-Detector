 ...
 *
 * O_RDWR means  read and write
 * O_CREAT means if path not exist, create it as a regular file
 * O_EXCL means ensure that this call creates the file
 * 
 ...
 * 
 * 这条语句用来干什么
 * forcing optimizer to noty re-order memory access across the barrier
 * 
 * example to do:
 * 
 int c(int *d, int *e) {
        ...
        d[0] += 1;
        r = e[0];
        // add or not add asm volatile("" ::: "memory")
        d[1] += 1;
        return r;
}
int main(void) {
 ...
 * 
 * after successful return , the old fd and new fd can be used interchangeably.
 * they refer to the same open file description and share file offset and file
 * status flags
 * 
 * if the file offset is modified by using lseek on one of the file descriptor, 
 * the offset is also for the other.
 * 
 * 
 * 一下三个dup2都需要成功，这3个的目的是什么？？用来干什么？？
 */
...
static void analyze(char** argv) {
  ...
  b_data[in_len] = 0xff; /* Intentional terminator. 故意中断*/
  ...
  for (i = 0; i < in_len; i++) {
    ...
    /* Perform walking byte adjustments across the file. We perform four
       operations designed to elicit（得出） some response from the underlying
       code. */
