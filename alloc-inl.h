 ...
 * ＡLLOC_CHECK_SIZE检查分配的大小(size)是否超过了MAX_ALLOC, MAX_ALLOC是什么？
 * 
 * ALLOC_OFF_TOTAL是什么？
 * 
 * #define ALLOC_CHECK_RESULT(_r, _s) do { \
...
static inline void* DFL_ck_alloc(u32 size) {
  ...
  if (!size) return NULL;
  mem = DFL_ck_alloc_nozero(size); // allocate an sized address 

/* void * memset ( void * ptr, int value, size_t num );
 * set the first num bytes of the block of memory pointed by ptr to the value
 * ptr = mem 
 * value = 0
 * size = num
 */
